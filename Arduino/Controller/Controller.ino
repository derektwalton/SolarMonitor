...
OneWire oneWire(5,6,7); // pin_I, pin_O, pin_OEn
...
void loop()
{
  ...
  if (! (count&0xf) ) {
    ...
    sprintf(topLine,"Temp C: %s",dtostrf(tempC,5,1,fstring));
    sprintf(botLine,"Temp F: %s",dtostrf(DallasTemperature::toFahrenheit(tempC),5,1,fstring));
